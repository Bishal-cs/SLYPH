# S.Y.L.P.H — Sophisticated Yet Lightweight Personal Helper
...
- **Sylph Mode** (default): The AI automatically decides whether to answer from its knowledge or search the web. A fast classifier routes each message to the right mode in ~200ms.
...
Say "generate an image of..." and Sylph creates it using Pollinations.ai. No API key, no cost, works instantly.

### Camera / Vision
Send a photo or use your webcam. Sylph analyzes the image and answers questions about it using Llama 4 Scout (free via Groq).
...
Put `.txt` files in `database/learning_data/` with any personal information you want Sylph to know. It reads them at startup and uses them as context when answering your questions.

### Session Persistence
Conversations are saved to disk. If you restart the server, your chat history is still there.

### Multiple API Keys
Set `GROQ_API_KEY_2`, `GROQ_API_KEY_3`, etc. in `.env` for automatic fallback. If one key hits its rate limit, Sylph switches to the next one automatically.
...
SYLPH_USER_TITLE=Sir
SYLPH_OWNER_NAME=Your Name
...
│   │   ├── brain_service.py   # Two-stage intent classifier (Sylph mode)
...
| `ASSISTANT_NAME` | No | `Sylph` | Name of the assistant |
| `SYLPH_USER_TITLE` | No | — | How AI addresses you (e.g. "Sir") |
| `SYLPH_OWNER_NAME` | No | — | Your name (so AI knows who it serves) |
...
| `POST /chat/sylph/stream` | Sylph mode — auto-routes, streaming |
