...
async function sendMessageWithImage(text, imgBase64) {
    ...
    try {
        ...
        const res = await fetch(`${API}/chat/sylph/stream`, {
            method: 'POST',
...
function setMode(mode) {
    currentMode = mode || 'sylph';
    if (btnSylph) btnSylph.classList.add('active');
...
async function sendMessage(textOverride) {
    ...
    const endpoint = '/chat/sylph/stream';
    if (activityList) {
